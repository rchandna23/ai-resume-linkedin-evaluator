...
import type { AnalysisResult, SectionFeedback, KeywordCoverage } from "../../models/AnalysisResult";
import { scoringConfig } from "../../config/scoringConfig";
import { computeMatchMetrics } from "./similarityEngine";

function clamp(value: number, min: number, max: number): number {
  return Math.min(max, Math.max(min, value));
}

function countQuantifiedBullets(experiences: Resume["experiences"]): number {
  let count = 0;
  const numberRegex = /\d/;
  for (const exp of experiences) {
    for (const bullet of exp.descriptionBullets) {
      if (numberRegex.test(bullet)) count += 1;
    }
  }
  return count;
}

function scoreResume(resume: Resume | null): { score: number; feedback: SectionFeedback[] } {
  if (!resume) {
    return {
      score: 0,
      feedback: [],
  ...
  const cfg = scoringConfig.resume;
  let score = cfg.base;
  ...
  if (quantified >= 5) {
    ...
    feedback.push({
      ...
      score: 5,
      comments:
        "Add more numbers (%, $, time saved, scale) to bullets so impact is clearly measurable.",
    });
  } else {
    feedback.push({
      section: "Impact & metrics",
      score: 3,
      comments:
        "Most bullets are responsibility-based. Add metrics to show concrete impact (e.g., \"Increased X by Y%\" ).",
    });
  }
...
function scoreLinkedIn(
...
): { score: number; feedback: SectionFeedback[] } {
  if (!linkedin) {
    return { score: 0, feedback: [] };
  }

  const cfg = scoringConfig.linkedin;
  let score = cfg.base;
  const feedback: SectionFeedback[] = [];
  ...
  if (linkedin.headline && linkedin.headline.length >= 20) {
    ...
    feedback.push({
      ...
      score: 8,
      comments:
        "Headline present. Ensure it combines role, key skills, and impact rather than just a job title.",
    });
  } else {
    feedback.push({
      section: "LinkedIn headline",
      score: 3,
      comments:
        "Write a stronger headline that combines your role, niche, and 2–3 core skills or outcomes.",
    });
  }
  ...
  if (linkedin.about && linkedin.about.length >= 150) {
    ...
    feedback.push({
      ...
      score: 8,
      comments:
        "About section present. Make sure it tells a concise story and highlights outcomes, not just responsibilities.",
    });
  } else {
    feedback.push({
      section: "About section",
      score: 3,
      comments:
        "Add an About section that summarizes your background, specialties, and what types of roles you’re targeting.",
    });
  }
...
function computeKeywordCoverage(
  jd: JobDescription,
  metrics: ReturnType<typeof computeMatchMetrics>
): KeywordCoverage {
  return {
    matched: [
...
export function scoreProfile(
  ...
  const metrics = computeMatchMetrics(resume, linkedin, jd);
  const cfg = scoringConfig.match;

  const requiredTotal = jd.skillsRequired.length || 1;
  const preferredTotal = jd.skillsPreferred.length || 1;

  const requiredCoverage =
    (metrics.requiredSkillMatches.length / requiredTotal) *
    cfg.weights.requiredSkillsCoverage;
  const preferredCoverage =
    (metrics.preferredSkillMatches.length / preferredTotal) *
  ...
  const combinedText =
    (resume?.summary ?? "") + " " + (linkedin?.headline ?? "");
  ...
  const jdMatchScore = clamp(
    cfg.base + requiredCoverage + preferredCoverage + headlineSummaryBoost,
    0,
    cfg.max
  );
