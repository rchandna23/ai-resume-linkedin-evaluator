...
export function parseJobDescription(rawText: string): JobDescription {
  ...
  const seniorityMatch = lower.match(
    /\b(junior|mid|senior|lead|principal|staff)\b/
  );

  const requirements: string[] = [];
  const responsibilities: string[] = [];
  const skillsRequired: string[] = [];
  const skillsPreferred: string[] = [];

  let current: "requirements" | "responsibilities" | "skillsRequired" | "skillsPreferred" | null =
    null;
  ...
  for (const line of lines) {
    ...
    if (
      /^required skills\b/.test(lowerLine) ||
    ...
    if (
      /^preferred skills\b/.test(lowerLine) ||
